...
import type { CollegeContext } from '../../domain/careerVoiceFlow';

export function PlacementOnboardingPage() {
  const navigate = useNavigate();
  const { userRole, setCollegeContext } = useAuth();

  const handleComplete = (context: CollegeContext) => {
    setCollegeContext(context);
    navigate('/placement', { replace: true });
  };

  const trackEvent = (name: string, meta?: Record<string, unknown>) => {
    console.debug('[PlacementOnboarding]', name, meta);
  };
  ...
  return (
    <div className="min-h-[100dvh] w-full flex flex-col items-center justify-center p-4 sm:p-6 lg:p-8 font-sans bg-[#f8fafc] text-[#0b111d] selection:bg-[#ea580c] selection:text-white">
      {/* Top Header Logo */}
      <header className="w-full max-w-[860px] flex items-center justify-between py-3 mb-4 px-1">
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-[11px] text-[#64748b] font-medium mt-0.5">Institutional Onboarding</span>
          </div>
        </div>
      </header>

      {/* Main Container: 760–900px */}
      <main className="w-full max-w-[860px]">
        <CollegeOnboardingStep
          roleType={userRole === 'college_management' ? 'college_management' : 'placement_team'}
