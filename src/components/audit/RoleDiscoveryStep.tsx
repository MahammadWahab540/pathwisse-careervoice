...
import { getRolesByStreamId, CareerRole } from '../../data/careerTaxonomy';
import { Sparkles, ArrowRight, ShieldCheck, ChevronRight, TrendingUp, DollarSign, Layers } from 'lucide-react';
...
export const RoleDiscoveryStep: React.FC<RoleDiscoveryStepProps> = ({
  ...
  const recommendedRoles = getRolesByStreamId(careerStreamId).slice(0, 3);

  const { isSpeaking, amplitude, speakText, stopSpeaking } = useVoiceInteraction({});

  const subtitleText = `I found 3 high-impact career directions for you, ${firstName || 'friend'}. Tap 'Deep Dive' on any path to explore day-to-day work.`;

  useEffect(() => {
    trackEvent('role_recommendations_viewed', {
      rolesCount: recommendedRoles.length,
      topRole: recommendedRoles[0]?.title,
    });
    speakText(subtitleText);
    return () => {
      stopSpeaking();
    };
  }, [trackEvent, recommendedRoles, firstName]);
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[10px] text-slate-500 font-semibold flex items-center gap-1">
                    ...
                    {role.demandLevel} Hiring Demand
                  </span>
                </div>
