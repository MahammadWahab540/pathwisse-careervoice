...
1. Career paths: ID, slug, title, role ID, source ID, content version, and checksum.
2. Roadmap skills: ID, career-path ID, slug, title, source ID, content version, and checksum.
3. Skill stages: ID, roadmap-skill ID, slug, title, order index, source ID, content version, and checksum.

Record a SHA-256 digest for the complete export. Do not copy service-role keys or
...
6. A human reviewer must approve every final row. A high score alone is not approval.
...
Allowed decisions are `APPROVED`, `REVIEW`, and `UNMAPPED`. Stage IDs must be
...
- assert exactly 110 manifest decisions and no duplicate CareerVoice keys;
- assert every manifest key exists in the published CareerVoice inventory;
