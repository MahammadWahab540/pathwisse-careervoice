...
async function runCli(): Promise<void> {
  ...
  if (!manifestPath || !inventoryPath) {
    console.error('Usage: npm run validate:careervoice-crosswalk-structure -- <manifest.json> <published-inventory.json>');
    process.exitCode = 2;
    return;
  }
  ...
  try {
    ...
    const result = validateCrosswalkManifest(manifest, { inventory });
    if (result.valid) {
      console.log(
        `Structurally valid CareerVoice crosswalk manifest (${EXPECTED_ROW_COUNT} rows). ` +
        'Authoritative catalog parentage and snapshot digest validation are still required.',
      );
      return;
    }
