...
async function checkAuditSessions() {
  // Let's try inserting an empty object to see what column constraints or columns exist
  const res = await supabase.from('audit_sessions').insert({}).select();
  console.log('audit_sessions insert result:', JSON.stringify(res, null, 2));
}
